...
class Octree {
	...
	getClosestTriangleFromPoint( point ) {
		const stack = [ ];
		const leafs = [ ];
		for ( let i = 0; i < this.subTrees.length; i ++ ) {
			stack.push( this.subTrees[ i ] );
		}
		...
		while ( node = stack.pop() ) {
			if ( node.subTrees.length === 0 ) {
				...
				for ( let i = 0; i < node.subTrees.length; i ++ ) {
					stack.push( node.subTrees[ i ] );
				}
			}
		}

		let closestLeaf = null;
		let closestDistance = 1e100;
		for ( let i = 0; i < leafs.length; i ++ ) {
			let distance = leafs[ i ].box.distanceToPoint( point );
		...
		for ( let i = 0; i < closestLeaf.triangles.length; i ++ ) {
			const triangle = closestLeaf.triangles[ i ];
			let target = new Vector3();
			triangle.closestPointToPoint( point, target );
			let distance = point.distanceTo( target );
			if ( distance < closestDistance ) {
				closestDistance = distance;
