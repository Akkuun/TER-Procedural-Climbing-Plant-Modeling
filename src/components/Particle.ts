...
class Particle {
    ...
    updateEllipsoidBody() {
        this.mesh.position.copy(this.x_rest);
        this.mesh.quaternion.copy(this.q_rest);
    }
    ...
    updateDimensions(newDimensions: THREE.Vector3) {
        ...
        otherUpdates(delta_time: number) {
            this.velocity = this.x_pred.clone().sub(this.x_rest).multiplyScalar(1/delta_time); // (Eq. 17)
    
            const s = 0.8; 
            this.x_rest = this.x_pred.clone()
...
function updateParticleGroup(delta_time : number, particleGroup : Particle[], gravity : THREE.Vector3, externalForce : THREE.Vector3, rootIndex : number=0) {
...
export { Particle, updateParticleGroup, particleRope };